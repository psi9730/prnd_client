...
import Storage, {getToken, KEYS} from '../../utils/Storage';
...
class CarDetailView extends Component<Props, State> {
  ...
  componentWillMount(){
    this.props.getCarRequest(this.props.match.params.id).then(()=>{
      ...
      console.log(endToStart,'end');
      console.log(nowToStart,'now');
      console.log("progress",(nowToStart) / (endToStart) * 100);
      const diff = this.getStringDateDiffInDays(now,end_at);
      this.setState({progress:  (nowToStart) / (endToStart) * 100,dateRemain: diff})}).catch((e)=>{
                ...
                <div className="mainImage">
                  ...
                  { this.props.status==="approved" ?
                    <div>
                      ...
                      <div className='abs'>
                        <Circle percent={this.state.progress} style={{height: 80, width: 80}} strokeWidth="8"
                                strokeColor="#2E7DE1"/>
                      </div>
                      <div className='abs3'>
                    ...
                    : (
                      this.props.status==="ended" ? <div><div className='abs2'>
